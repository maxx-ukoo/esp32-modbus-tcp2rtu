...
esp_err_t ota_list_files_handler(httpd_req_t *req)
{
	...
	while ((ent = readdir(dir)) != NULL)
	{
		...
		if (CONFIG_EXAMPLE_WEB_MOUNT_POINT[strlen(CONFIG_EXAMPLE_WEB_MOUNT_POINT) - 1] != '/')
			strcat(entrypath, "/");
...
/* Receive .Bin file */
esp_err_t ota_upload_firmware_handler(httpd_req_t *req)
{
...
int calculate_data_lenght(const char *data, int data_len, const char *pattern, int pattern_len)
{
	int cursor = 0;
	static int maches = 0;
...
esp_err_t upload_file_handler(httpd_req_t *req)
{
	...
	if (boundary_len > 1)
	{
		...
		if (boundary_start_p)
		{
			...
			boundary_start_p = (char *)&boundary_buff;
